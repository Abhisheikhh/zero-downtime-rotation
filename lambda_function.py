...
def set_secret(service_client, arn, token):
    ...
    # Removed the check for AWSCURRENT to allow the lambda to self-heal desynchronized passwords
...
def finish_secret(service_client, arn, token):
    ...
    and immediately expiring the previous user's password in the database.
    """
    ...
    # Get the newly promoted secret to find out which user is now active
    new_dict = get_secret_dict(service_client, arn, "AWSCURRENT")
    new_username = new_dict['username']

    logger.info("finishSecret: New active user is %s. Password rotation complete." % new_username)
